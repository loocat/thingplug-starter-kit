...
API.createAE = function (config, cb) {
  let headers = {
    'X-M2M-Origin': 'S',
  ...
  let path = `/~/${config.servID}/${config.version}`;
  httpClient.createRequest('POST', path, headers, body).request((err, res) => {
    if (err) {
      cb(err);
    }
    else {
...
API.getLatestContainer = function(nodeID, containerName, cb){
  ...
  var path = `/~/${config.servID}/${config.version}/ae-${nodeID}/cnt-${containerName}`;
...
API.reqMgmtCmd = function(nodeRI, command, payload, cb){
  ...
  var path = util.format('/%s/%s/mgmtCmd-%s_%s'
